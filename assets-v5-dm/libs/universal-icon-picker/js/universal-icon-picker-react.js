 ...
 */
(function(wp) {
    const { createElement, useState, useEffect, useMemo, useCallback, Fragment } = wp.element;
    ...
    function IconPickerModal({ isOpen, onClose, onSelect }) {
        ...
        return createElement('div', { className: 'modal fade show', style: { display: 'block' }, role: 'dialog' },
            createElement('div', { className: 'modal-dialog modal-xl' },
                // --- CORRECTED LAYOUT ---
                // The modal-content itself is now a flex container, which gives us proper control over its children.
                createElement('div', { className: 'modal-content', style: { height: '85vh', display: 'flex', flexDirection: 'column' } },
                    ...
                    // This body will grow to fill the available space.
                    createElement('div', { className: 'modal-body', style: { flex: '1 1 auto', overflow: 'hidden' } },
                        createElement('div', { className: 'row h-100' },
                            ...
                            // This column will contain the search bar (fixed) and the icon grid (scrollable).
                            createElement('div', { className: 'col-md-9 d-flex flex-column h-100' },
                                ...
                                // This div now correctly fills the remaining space and handles its own scrolling.
    ...
    function IconPickerWrapper(props) {
        ...
        return createElement(Fragment, null,
            ...
            isModalOpen && createElement(Fragment, null,
                createElement(IconPickerModal, {
                    isOpen: isModalOpen,
                    onClose: () => setIsModalOpen(false),
                    onSelect: handleSelect,
                }),
                createElement('div', { className: 'modal-backdrop fade show' })
            )
        );
    }

    window.myBlockTools = window.myBlockTools || {};
    window.myBlockTools.IconPickerButton = IconPickerWrapper;
