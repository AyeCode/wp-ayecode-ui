<script>
	...
	function aui_init_flatpickr(){
		if ( typeof flatpickr === "function" && !$aui_doing_init_flatpickr) {
			...
			<?php
			if ( ! empty( $flatpickr_locale ) ) {
				?>
				try{flatpickr.localize(<?php echo $flatpickr_locale; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>);}catch(err){console.log(err.message);}<?php } ?>
			document
